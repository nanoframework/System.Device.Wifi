...
else
{
    ...
    git add "source\Windows.Devices.WiFi\Properties\AssemblyInfo.cs"
...
if ($env:APPVEYOR_REPO_BRANCH -match "^dev*" -or $env:APPVEYOR_REPO_TAG -eq "true")
{
    ...
    # new branch name
    $newBranch = "$env:APPVEYOR_REPO_BRANCH-nfbot/update-version/Windows.Devices.WiFi/$env:GitVersion_NuGetVersionV2"
    ...
    $assemblyFiles = (Get-ChildItem -Path ".\*" -Include "win_dev_wifi_native.cpp" -Recurse)
    ...
    {
        $commitMessage = "Update Windows.Devices.WiFi version to $env:GitVersion_NuGetVersionV2"
